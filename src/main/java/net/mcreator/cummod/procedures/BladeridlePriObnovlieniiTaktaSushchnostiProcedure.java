...
public class BladeridlePriObnovlieniiTaktaSushchnostiProcedure {
		...
		if (!world.isClientSide()) {
			if (!world
				...
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = new BladerattackEntity(CummodModEntities.BLADERATTACK.get(), _level);
				...
				if (!world.getEntitiesOfClass(BladerattackEntity.class, AABB.ofSize(new Vec3(posX, posY, posZ), 16, 16, 16), e -> true).isEmpty()) {
					{
						Entity _ent = ((Entity) world
								.getEntitiesOfClass(BladerattackEntity.class, AABB.ofSize(new Vec3(posX, posY, posZ), 16, 16, 16), e -> true).stream()
								.sorted(new Object() {
					...
					((Entity) world.getEntitiesOfClass(BladerattackEntity.class, AABB.ofSize(new Vec3(posX, posY, posZ), 16, 16, 16), e -> true)
							.stream().sorted(new Object() {
								Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
									return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
								}
							}.compareDistOf(posX, posY, posZ)).findFirst().orElse(null)).setDeltaMovement(new Vec3(velocityX, velocityY, velocityZ));
					if (((Entity) world.getEntitiesOfClass(BladerattackEntity.class, AABB.ofSize(new Vec3(posX, posY, posZ), 16, 16, 16), e -> true)
							.stream().sorted(new Object() {
								Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
									return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
								}
